...
app.config["SECRET_KEY"]="gdfgdfgh151g1drg"

class RegisterForm(FlaskForm):
	"""自定义的注册表单模型类"""
	#  						名字			验证器/验证器
	# DataRequired 保证数据必须填写，并且不能为空
	user_name=StringField(label=u"用户名",validators=[DataRequired(u"用户不能为空")])
...
def register():
	# 创建表单对象,如果是post请求，前段发送的数据flask会吧数据在构造form对象的时候，存到对象中
	form=RegisterForm()
	# 判断foem总的数据是否合理
	# 如果form中的数据完全满足所有的验证器，返回真，否则返回假
